...
**STEP 4b · LEN PAYLOAD CLASSIFICATION (inside a protobuf message).** Same precedence as the body-level pivot, and easy to get backwards: `Protobuf.looksNested()` is tried **before** the text test, because a nested message carrying tokens/IDs/URLs is >90% printable and a printable-first test flattens its fields into one mangled value. `looksNested` requires a clean whole-buffer parse that re-encodes byte-identically, field numbers ≤ 512, and — when the payload is clean text end to end — at least one LEN field, so short text that parses by luck (`"PB"` → `10: 66`) stays a string. The string test here is `asCleanString` (rejects any C0 byte other than tab/CR/LF), **not** the tolerant 90% `asPrintable`, which remains the body-level gate. Anything else renders as hex. Enforced by `ProtoNestingTest`; the rule holds in all four renderers (`ProtoText`, `ProtoNodes`, `SchemaRenderer`, `Protobuf.render`).
